...
const expressions = {
    ...
    dni: /^[0-9]{8}[A-Za-z]$/,
    relation: /^[a-zA-ZÀ-ÿ\s]{2,40}$/ // Letras, con espacios, mínimo 2 y máximo 40 (incluye hasta 4 espacios).
};
...
$('.form__input').on('keyup blur', function (e) {
    switch (e.target.name) {
        ...
        case "phone":
            ...
            break;
        case "relation":
            validateField(expressions.relation, e.target, 'relation');
            break;
    }
});

// Validar campo de selección en el formulario de agregar
$('#select-principalTutor').on('change', function () {
    validateSelect($(this), '#group__principalTutor');
  });
